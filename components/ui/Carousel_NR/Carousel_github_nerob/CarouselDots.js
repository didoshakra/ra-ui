...
class CarouselDots extends Component {
  ...
  calculateButtonValue = () => {
    ...
    return this.props.value >= 0
      ...
      : this.props.value +
          numberOfSlides *
            Math.ceil(Math.abs(this.props.value / numberOfSlides));
  };

  renderCarouselDots() {
    if (this.props.thumbnails) {
      const dotsLength = isNaN(this.props.number)
            ...
            <button
              className={classnames("BrainhubCarousel__thumbnail", {
                "BrainhubCarousel__thumbnail--selected":
                  index === this.calculateButtonValue() % dotsLength,
              })}
              type="button"
          ...
          <button
            className={classnames("BrainhubCarousel__dot", {
              "BrainhubCarousel__dot--selected":
                i === this.calculateButtonValue() % this.props.number,
            })}
            type="button"
