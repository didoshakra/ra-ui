...
class Carousel extends Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (
      this.getProp("autoPlay") !== this.getProp("autoPlay", prevProps) ||
      valueChanged
    ) {
      this.resetInterval();
    }
  ...
  /* ========== tools ========== */
  getCurrentValue = () =>
    this.props.infinite ? this.props.value : this.clamp(this.props.value);

  getNeededAdditionalClones = () =>
    Math.ceil(
      (this.getCurrentValue() - this.state.infiniteTransitionFrom) /
  ...
  getClonesLeft = () =>
    config.numberOfInfiniteClones + this.getAdditionalClonesLeft();
  getClonesRight = () =>
    config.numberOfInfiniteClones + this.getAdditionalClonesRight();

  getAdditionalClonesOffset = () =>
    -this.getChildren().length *
    this.getCarouselElementWidth() *
  ...
  checkIfValueChanged = (prevProps) => {
    const currentValue = this.getProp("infinite")
      ? this.props.value
      : this.clamp(this.props.value);
    const prevValue = this.getProp("infinite")
      ? prevProps.value
      : this.clamp(prevProps.value);
  ...
  getActiveSlideIndex = () =>
    this.getProp("infinite")
      ? this.getCurrentSlideIndex() +
        this.getClonesLeft() * this.getChildren().length
      : this.getCurrentSlideIndex();
  ...
  onResize = throttle(() => {
    ...
    const arrowRightWidth =
      this.arrowRightNode && this.arrowRightNode.offsetWidth;
    const width =
      this.node.offsetWidth - (arrowLeftWidth || 0) - (arrowRightWidth || 0);
  ...
  onMouseMove = (e) => {
    ...
    if (this.state.dragStart !== null) {
      this.setState((previousState) => ({
        dragOffset: this.getProp("rtl")
          ? previousState.dragStart - pageX
          : pageX - previousState.dragStart,
      }));
    }
  };
  ...
  onMouseUpTouchEnd = (e) => {
    if (this.state.dragStart !== null) {
      e.preventDefault();
      if (
        this.getProp("draggable") &&
        Math.abs(this.state.dragOffset) > config.clickDragThreshold
      ) {
   ...
   */
  changeSlide = (value) =>
    this.props.onChange(this.getProp("infinite") ? value : this.clamp(value));

  nextSlide = () =>
    this.changeSlide(this.getCurrentValue() + this.getProp("slidesPerScroll"));
  ...
  getNearestSlideIndex = () => {
    ...
    if (this.getProp("keepDirectionWhenDragging")) {
      if (this.state.dragOffset > 0) {
        slideIndexOffset = -Math.ceil(
          this.state.dragOffset / this.getCarouselElementWidth()
        );
      } else {
        slideIndexOffset = -Math.floor(
          this.state.dragOffset / this.getCarouselElementWidth()
        );
      }
    } else {
      slideIndexOffset = -Math.round(
        this.state.dragOffset / this.getCarouselElementWidth()
      );
    }
  ...
  getCarouselElementWidth = () =>
    this.getProp("itemWidth") ||
    this.state.carouselWidth / this.getProp("slidesPerPage");
   ...
   */
  getTransformOffset = () => {
    const elementWidthWithOffset =
      this.getCarouselElementWidth() + this.getProp("offset");
    ...
    return (
      dragOffset -
      currentValue * elementWidthWithOffset +
      additionalOffset -
  ...
  renderCarouselItems = () => {
    ...
    const trackLengthMultiplier =
      1 +
      (this.getProp("infinite") ? numberOfClonesLeft + numberOfClonesRight : 0);
    const trackWidth =
      this.state.carouselWidth * children.length * trackLengthMultiplier;
    const animationSpeed = this.getProp("animationSpeed");
    const transitionEnabled = this.state.transitionEnabled;
    const draggable =
      this.getProp("draggable") && children && children.length > 1;
    ...
    const handleAutoPlayEvent = (action) =>
      isAutoPlay && isStopAutoPlayOnHover ? action : null;

    return (
      <div className="BrainhubCarousel__trackContainer">
        <h3>Carousel https://github.com/brainhubeu/react-carousel</h3>
        <ul
          ...
          {slides.map((carouselItem, index) =>
            ...
            [null, undefined].includes(carouselItem) ? null : (
              <CarouselItem
                ...
                isDragging={
                  Math.abs(this.state.dragOffset) >
                  this.props.minDraggableOffset
                }
                isDraggingEnabled={
                  this.props.draggable || this.props.clickToChange
                }
              >
                {carouselItem}
              </CarouselItem>
  ...
  renderArrowLeft = () => {
    ...
    if (this.getProp("arrowLeft")) {
      if (!disabled) {
        return this.renderArrowWithAddedHandler(
          this.getProp("arrowLeft"),
          this.prevSlide,
          "arrowLeft"
        );
      }
      const arrow = this.getProp("arrowLeftDisabled")
        ? this.getProp("arrowLeftDisabled")
        : this.getProp("arrowLeft");
      return this.renderArrowWithAddedHandler(
        arrow,
        this.prevSlide,
  ...
  renderArrowRight = () => {
    ...
    if (this.getProp("arrowRight")) {
      if (!disabled) {
        return this.renderArrowWithAddedHandler(
          this.getProp("arrowRight"),
          this.nextSlide,
          "arrowRight"
        );
      }
      const arrow = this.getProp("arrowRightDisabled")
        ? this.getProp("arrowRightDisabled")
        : this.getProp("arrowRight");
      return this.renderArrowWithAddedHandler(
        arrow,
        this.nextSlide,
